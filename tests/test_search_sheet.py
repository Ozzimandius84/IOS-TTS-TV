...
PROOF = REPO / "scratch-j13" / "proof.mjs"
...
def test_lib_rs_carries_two_lines_and_no_fourth_command():
    ...
    assert "frank_search" not in lib, "lib.rs must not name the command"
...
def test_the_engine_is_the_one_lookup_js_already_named():
    """One press, one URL, whichever of the three landings takes it."""
    ttstv = _ttstv()
    if ttstv is None:
        pytest.skip("no TTSTV checkout beside this one")
    lookup = (ttstv / "reader" / "lookup.js").read_text("utf-8")
    m = re.search(r'const SEARCH = "([^"]+)";', lookup)
    assert m, "lookup.js has no SEARCH constant"
...
def test_the_shim_runs():
    """`scratch-j13/proof.mjs` slices SEARCH_JS out of search.rs and the search
    control out of lookup.js and RUNS them -- including in the order Tauri
    actually injects scripts, where `window.__TAURI__` does not exist yet."""
    if _ttstv() is None:
        pytest.skip("no TTSTV checkout beside this one")
    if not PROOF.is_file():
        pytest.skip("scratch-j13/proof.mjs is untracked scratch and is not here")
    r = subprocess.run([NODE, str(PROOF)], cwd=REPO, capture_output=True, text=True)
