...
**TTSTV (`FRANK`)** — two commits, the ONLY way the fix can reach the phone (`shell/` here is never edited by hand; `library/library.html` is its master):
- `a194138` `library/library.html` — `refresh()`'s static branch (`/state` failed → fetch `library.json`) reads the file into a local and assigns **only if `libraryBooks` is still null after the await**; the same guard on the W-web `../books/index.json` road.
- `bfaf2eb` `shell/` rebuilt by `design/export.py --shell`, `--verify` **0 DRIFT, 208 routes**.

**Phone (`main`)**
- `76cada8` — `shell/` re-imported from TTSTV (`ttstv-shell-v60`, 75 files, 2,851,166 bytes; 22 files changed — it also carries every lane's 15–26 Sep shell work the phone never had: D-voice, D-hands, W-web, F-feel, the float trio). `tests/phone_shelf.mjs` (new). `tools/import_shell.py`: its pre-flight refused a tree whose masters live only under `design/reader/` (`float.js`, `floatdoor.js`, `wordclock.js`) although `ship.py` builds them fine — it now accepts a module file OR a `design/` master. `shell/library/samples/poems.zip` (2.2 MB, a hand-made W4 leftover TTSTV never produced) gone with the import; the manifest is now exactly what TTSTV wrote + `index.html`.
- `37ad4c3` — `tests/phone_lookup.mjs` (new). `tools/phone.sh`: `--register` (BUILD's owed device-registration line) and `--sim [NAME]` (the card's exact `tauri ios build --debug --target aarch64-sim --ci`, then `simctl install` on the booted device — **never Simulator.app**).
- **`shell/reader/reader.html` hand-edit: DROPPED** (`git checkout --`). A by-hand `../books/`→`../../books/` in a generated file; wrong for the phone target (in the bundle the shell IS the root and `ship.build(target="phone")` re-aims to `../books/` itself) and overwritten by the import anyway.
- `28f4d6b` was a MISTAKE — `/tmp/report.md` on the VM was another lane's file (E-parser's, owned by `nobody`, my write to it was refused) and got prepended to this STATUS.md. Undone by the commit carrying this report; nothing else in that commit.

### 2. Verified — and how

| claim | how |
|---|---|
| **THE CAUSE, measured.** 40 rows behind `book_list` → `placeDeviceBooks` puts 40 into `libraryBooks` at **t=229 ms, 40 tiles**; at **t=258 ms libraryBooks=1, tiles=1** — `refresh()` had entered its null branch when `/state` failed (at once on frank://), awaited `library.json`, and `if (res.ok) libraryBooks = await res.json()` replaced the 40 with the dev shelf's one sample row (`poems`). With no `library.json` in the bundle the `[]` re-checked null, so the 25 Sep phone showed 0; with a shelf cache `libraryBooks` was never null, so the branch was skipped — exactly NOT-DONE's "no tiles unless a saved shelf cache exists" | container Chromium 393×852, the REAL `shell/`, the phone's init scripts in lib.rs's order read out of the Rust (`HOST_JS`, `BOOKS_JS`, `SYNC_JS`, `DICT_JS`, `LOOKUP_JS`, `INBOX_JS`) over a stand-in `invoke`; a 20 ms poll of `libraryBooks.length` / tile count |
| **tiles == books** | `node tests/phone_shelf.mjs` on the imported shell: **tiles 40 of 40**, `TTSTVContext {kind: phone, phone: true, device: true, bench: false}`, storage line "40 books", 3 tile rects **150×335**, 0 page errors; 0 books → 0 tiles and the phone's own empty sentence. `library.json` is served 150 ms late so the race is deterministic: **old shell 1/40 (FAIL), new shell 40/40** |
| **one word looked up** | (a) the pack: `dictionary/pack.py::entry` on the Mac's `TTS_DATA/dictionary/packs/la.sqlite` (842,122 lemmas, 3,703,110 forms, opens in 14 ms): `arma` → noun, "arms, weapons of war…" **1.7 ms**; `virumque`, `cano`, `Troiae`, `vocābant` all answered, 1.8–2.7 ms. (b) the page: `node tests/phone_lookup.mjs` — real `lookup.js` + real `DICT_JS`, `dict_lookup` answering that exact entry: the card shows `arma`, `noun`, the first gloss, **4 glosses**, asked the door **once** with `"lang":"la"`; `zzzz` → "No entry for “zzzz”…", not "Looking it up…"; 0 page errors |
| the shell is TTSTV's | `--shell --verify` 0 DRIFT / 208 routes before the import; `shell.manifest.json` rewritten by the import (source `2d03858`, then-HEAD, which contains `a194138`+`bfaf2eb`) |
| nothing else broke | phone `pytest tests` (container): **113 passed, 2 skipped, 8 failed** — 3 are my container tarball lacking `src-tauri/ios/*.m` (not staged), 5 pre-existing and untouched by this lane: `test_phone_loop` ×2 (project.yml's PATH now names Homebrew — BUILD's Air change), `test_phone_shell` (lib.rs names six shell files), `test_google_link`, `test_pair_link` (the 5811e43 pbxproj/scheme regen). `tools/phone.sh`'s three tests pass with the new flags |
| nothing rewritten by a probe | `md5sum languages/catalogue.json` = `617c7c8c68f42d6c86b1d651abeef326` at the gate and after the last TTSTV commit; `TTSTV_DATA_HOME`/`TTS_DATA` pointed at `/tmp` for every export/import; no server started |
| **the simulator build** | **NOT RUN, cannot be from here**: the bridge is a Linux VM (no xcodebuild) and Terminal is click-tier. `tools/phone.sh --sim` is the one command; §8 |

### 3. Judgment calls
- **The fix went into TTSTV, not the phone repo, and that is the lane's rule kept, not broken.** `shell/` "is NEVER EDITED BY HAND" (README); the page is mastered by `library/library.html`; nobody was in `library/` or dirty on any shell file (checked `git status` before `--shell`). A phone-side workaround (answering `library.json` from Rust with the door's rows) would have left the race in place and stripped `device:true` off the rows. Named in §4.
- **`has_dictionary: false` ×40 is not a fault.** G-LANG (11 Sep): a book carries no `dictionary.json`; the dictionary is per-language SQLite through `TTSTVHost.dict`. "No entry" today means **no pack on the phone yet** — Drive's `Frank/languages/` has la/en/fr/grc/sa/de (14 Sep) and `languages.json`, so it is one press in Settings ▸ Languages. Proved here: both halves that can be proved headless; the press is Osca's (§8).
- **The dirty `reader.html`: drop, not commit.** A hand edit in a generated file that the very next import overwrites, and `../../books/` is the clean/ depth, not the bundle's.
- **`poems.zip` left deleted.** Not TTSTV output, not `DEV_ONLY`, so the manifest gate should never have passed it; the dev shelf's `library.json` was held and put back as import_shell promises.
- **The lookup test's bench page is served by the test, not added to `shell/`** — same reason.

### 4. Boundary check
Touched — phone repo: `shell/` (import only), `shell.manifest.json`, `tools/import_shell.py`, `tools/phone.sh`, `tests/phone_shelf.mjs`, `tests/phone_lookup.mjs`, `STATUS.md`. **TTSTV: `library/library.html` and its `shell/` rebuild — two repos, one fix, said here in my own words**: the phone's shelf is TTSTV's page, and the phone repo may not carry an edit of it. `core/` untouched. Found dirty in TTSTV and LEFT ALONE: `Frank/FRANK.md`, `cloud/*`, `desktop/SHIPPING.md`, `desktop/tools/rebuild.sh`, `desktop/tools/dmg.sh` (untracked), `settings/tests/test_cloud_endpoint_tab.py`, `studio/{cli,cloud,languages,serve}.py`, `studio/tests/*` — none is a shell file, which is what made `--shell` safe.

### 5. Footprint
Container (thrown away): the phone shell + Rust sources + tests (~5 MB), pytest. **On the Mac, `_to_delete/` (this shell cannot delete):** phone repo `shell.1790432113` (the pre-import shell, 5.0 MB) + `index.lock`/`HEAD.lock` ×~8; TTSTV `HEAD.lock`, `index.lock`, `next-index-7.lock`, `next-index-16.lock` (stamped `.1790432033`…). `scratch-probe/` holds `phone-b.tgz`, `phone-c.tgz`, `phone-py.tgz`, `library.fixed.html`, `phone.sh.new`, `status.prev.md` (staging copies, ~5 MB). No SSD write, nothing under `~/Desktop`, no venv.

### 6. Requests
1. **TTSTV `library/tests`** — a node test for the race would belong beside the master (`test_library_phone_shelf.py`'s rig); this lane proved it from the phone repo over the built shell instead. Ten lines for whoever next holds `library/`.
2. **`tests/test_phone_loop.py` ×2, `test_phone_shell.py`, `test_google_link.py`, `test_pair_link.py`** are red at HEAD for reasons that predate this lane (BUILD's project.yml PATH, the 5811e43 regen, lib.rs naming shell files). Somebody's, named.

### 7. Known gaps
- **Not run on a phone or a simulator.** The imported shell and the fix are proved in Chromium over stand-in doors; WebKit was not driven (no Mac shell).
- The pack on the phone is **not installed** — nothing here pulled 71 MB onto the device. The card will say "No entry" until Settings ▸ Languages ▸ Latin is pressed.
- `tools/phone.sh --sim` and `--register` are **written and `bash -n`-clean, never executed** (no macOS here).
- The profile on the real phone still expires **3 Oct**.

### 8. Next — Osca's presses, in order
1. `cd "~/Documents/RUNNERS/TTSTV_IOS/IOS TTS TV" && tools/phone.sh --sim` — sim debug build + install on the booted device, no Simulator.app; then `xcrun simctl launch booted com.ttstv.frank` and read `scratch-probe/probe.log`. Expect the shelf to draw every installed book.
2. On the phone (after the Trust tap): Settings ▸ Languages ▸ **Latin** ▸ add (71 MB from Drive), open a Latin book, press a word: the card, not "No entry".
3. `git push origin FRANK` (a194138, bfaf2eb among the unpushed) and `git push` in the phone repo.

### 8b. Commit check
Phone: `76cada8` (24 files: shell/ ×21 + manifest + import_shell.py + phone_shelf.mjs), `37ad4c3` (phone_lookup.mjs, phone.sh), `28f4d6b` (STATUS.md — WRONG content, see §1), then this report's commit (STATUS.md, replacing the whole 28f4d6b prepend) — each `git show --stat` listed exactly the named paths; every commit by pathspec; new files `git add -- <path>`ed first. TTSTV: `a194138` (library.html alone), `bfaf2eb` (shell/MANIFEST.json + shell/library/library.html alone). Locks moved: TTSTV `HEAD.lock` (713 s), `index.lock` (96 s), `next-index-7.lock`, then `HEAD.lock`/`next-index-16.lock` after each commit; phone `index.lock`/`HEAD.lock` after each — all in the two `_to_delete/`. Nothing amended, nothing pushed (no GitHub login on the bridge).

### 9. Status line
`phone · B-phone wave 8 · 26 Sep · tiles == books (40/40) proved on the real shell over the real init scripts; the la pack answers in 1.7 ms and the card draws it; reader.html dropped; the sim build is tools/phone.sh --sim, Osca's press`
...
### 1. Built
**Phone repo (`main`)**
- `src-tauri/src/lib.rs`, beside the book door, because the books folder is the book door's and `pull.rs` has never named a path of its own (`tests/test_sync_pull.py` holds that rule): `BOOK_TOPUP` — `library/import.js`'s `TOPUP` map, rel for rel, `{"cover.jpg": "has_cover"}`, an **allowlist and the whole of it** — `book_topup_flag`, `book_topup_names`, `book_live` (installed, AND at the hash the job names; asked again at the commit rather than remembered), `book_topup_dest`, `book_topup_aside`, `book_topup_have` (the resume's question asked of the **live** folder, so a cover is paid for once), `book_topup_write` (streamed to a dot-name, then RENAMED inside the book's own folder), `book_topup_commit` (the installed row gains `has_cover: true` for every allowlisted name the folder really holds; the JOB's row is not written). `content_type`: `Some("jpg") => "image/jpeg"`.
- `src-tauri/src/pull.rs`: `KIND_TOPUP` and `TopUp` — four calls into the above and nothing else, as `BookDoor` is; `door_for`'s third arm; `check_job`'s kind arm (`books, language or topup`) and the top-up's own two rules (ONE file, and it is on the allowlist), refused before a byte; `queues()` — a top-up waits for a running pull as a language pack does, and a books job is still refused by one; `enqueue`'s "once per set" is now once per (kind, set). `installed()` answers `None` **by design** (§3).
- `tests/test_sync_pull.py`: the rule test reads pull.rs's `use crate::{…}` as a SET and names the four `book_topup_*`; and holds the top-up's own rules — it writes through `book_topup_dest`, never `book_part`; `BOOK_TOPUP` is import.js's map; a cover is served as a picture.
- `shell/` re-imported (v55, `f045706`).

**TTSTV (`FRANK`)**
- `library/drive.js` — **the top-up**: `SYNC_TOPUP` (import.js's map for a page that has not loaded import.js), `syncTopUps` (import.js's rule when the page has it, its own copy when it has not), `syncTopUpJob` (one file a book; `meta` is THIS DEVICE'S row), `syncStartJob` (the books, then the top-ups; `topUp`/`refused` stripped from the wire); `syncJob` carries `topUp`; `syncHandOff` answers `topped`.
- `library/drive.js` — **the ledgers**: `syncMarksLedger`, `syncPositionsLedger`, `syncSettingsLedger` lifted out of `runDriveSync` unchanged, which now calls them; `syncDefaultStore` (the reader's four keys, named here for the app's ask and nothing else); `syncLedgers`; `syncAuto` starts the pull and then merges the three.
- `library/tests/test_sync_topup.py` (new, 5).

### 2. Verified — and how
- **Rust, 26 passed / 1 failed** (`cargo test` in the Cowork container, rustc 1.95): `pull_tests` **12 → 16**, `dict_tests` 9, `book_tests` — on a harness crate whose lib is lib.rs's items **extracted VERBATIM by brace-matching** (the 7 consts and 24 functions the two modules use; none of them touches `tauri`) plus the **real** `pull.rs` and `dict.rs`, against the **real** serde 1 / serde_json 1 / log 0.4 / **ureq 2.12** / rusqlite 0.37 (bundled) / flate2 1 from crates.io — not mocks this time: `static.crates.io` and `index.crates.io` answer 200 from the container (crates.io's own API is 403; GitHub is 403). The 1 failure is the harness's own: `dict_tests::the_pack_door_is_three_commands_and_nothing_else` asserts the four `tauri::command`s of dict.rs, and those four items are the only thing cut from the harness copy (they need `tauri`). Not `cargo test` on the real crate — owed (§7).
- **The four new Rust tests, in words.** *The cover written in and the row flipped*: `cover.jpg` on disk; the row's `has_cover` `false → true` with `title`, `words`, `hash` untouched; `book_installed` unchanged; and **every other file in the folder byte-identical**, held by a comparison of the whole tree before and after (`after.len() == before.len() + 1`, no `.topup-*` left, **no `.part/hamlet@aaaa` ever opened**, no `.old/`). *Refused*: seven `rel`s — `book.meta.json`, `chapters/c001.txt`, `cover.png`, `cover.jpg.exe`, `a/cover.jpg`, `../cover.jpg`, `.meta.json` — and a second file, each a sentence, **nothing asked of the network**; a book not on the device, and a book here at another hash, refused at the write; and after all of it `tree(live)` equals what it was. *Not fetched twice*: the same job run again asks the network for nothing and the row is still `true`. *The small pieces*: the flag map, `queues`, and a kind that is not one.
- **RED controls, 7 mutations, 6 caught**: `installed()` reporting the real hash (3 fail), `commit` writing the job's row instead of patching this device's (3), the hash guard off in `book_live` (2), the allowlist off in `check_job` (2), the one-file rule off (2), `queues` without `topup` (2). The seventh — the allowlist in `book_topup_dest` alone — is **not** caught, because `check_job` refuses the same rel first; it is belt-and-braces and kept for the door's own sake, and that is said here rather than dressed up.
- **node/pytest, TTSTV**: `library/tests` + `settings/tests` **379 → 384 passed** in the container, and the failing/erroring set is **byte-identical before and after** (43 failed, 26 errors — chromium/playwright absent there, and two `test_sync_pull.py` line tests that fail on today's DATE, not on this change: they pin `17:41` and the row now says `11 Sep`). The 5 new tests are **RED on the pre-change `drive.js`: 5 of 5**.
- **node/pytest, phone**: `tests/` **116 passed, 4 failed** before and after — the same four this repo has carried (`test_pair_link` scheme, `test_phone_loop` ×2, `test_phone_shell`). The rule test `the_runner_writes_only_through_the_book_doors_own_functions` is RED on the new pull.rs with the old assertion, which is what sent the top-up's writes into lib.rs (§3).
- **the loop, against a shadow tree**: another lane holds six shell files dirty, so nothing was pushed to `clean/` here. In a `git archive` of HEAD `9a8fb74` (6 dirs, 57 MB, in the VM's session home, never the repo): `export.py --shell` 70 files → `--shell --verify` **VERIFY 65 identical, 5 known-and-named, 0 DRIFT; ROUTES 184 / 0** → `publish_shell --no-bump` **exit 0**, 67 files, 2361.3 KB. `clean/library/drive.js` differs from the module's by exactly one line, `ship.py`'s GENERATED header. `md5sum languages/catalogue.json` `304915f7c7f6ec6e6f3a38553d442e7c` at the gate and at the last commit. No server started, no route pressed.
- **the phone shell**: imported from that same archive — 68 files, 2,418,168 bytes, `ttstv-shell-v55`; `shell/library/drive.js` is the module's file byte for byte; `src-tauri/google.json` and `tauri.conf.json` untouched.
- **not verified**: the simulator; the real phone; a real Drive top-up or a real LAN one; `cargo test` on the real crate; ureq/rustls on `aarch64-apple-ios` (unchanged since G-SYNCBG); WebKit drawing an `image/jpeg` (it is the type the Mac has always served, and that is the reason to send it, not a proof).

### 3. Judgment calls
- **The top-up's writes went into `lib.rs`, not `pull.rs`.** Written first as a `TopUp` door that did its own `fs::rename`/`File::create` inside pull.rs — and `tests/test_sync_pull.py` refuses that by name ("pull.rs writes on its own"). The rule is right and it is not mine to reverse: the books folder is the book door's and pull.rs is the downloader. So the four `book_topup_*` live beside `book_write_from` and `book_commit`, `TopUp` is four calls, and the rule test grew rather than bent — its import-list assertion is now a SET, so the next function to join that list is a one-word edit and not a re-typed line.
- **`TopUp::installed()` answers `None`, always.** The runner skips a book whose door reports the job's hash — and a top-up's hash IS the installed one. Reporting it truthfully would skip every top-up there is. The question is asked instead at the moment it has to be true, by `book_live`, at the write and again at the commit.
- **NEVER a `.part/`.** `book_commit` swaps a whole folder; a `.part/` holding only `cover.jpg` would replace the book with a folder of one file. The prompt says this and it is the whole reason the verb is in-place.
- **What a top-up may write is a LIST OF NAMES, not a rule.** This is the one verb that writes into a book already on the shelf, so `BOOK_TOPUP` is an allowlist — `import.js`'s own map, rel for rel, so the two cannot mean different things. A `rel` off it is refused before a byte and again at the write.
- **The commit patches THIS DEVICE'S row and does not write the job's.** The row in `books/<slug>/.meta.json` was written when the book was pulled here; a row planned from the Mac's `library.json` could only be older, and writing it whole would silently roll back anything this device knows about the book. One field, and the field is set from what the folder really holds — not from what the job said it would hold.
- **A top-up QUEUES behind a running pull** (`queues()`), because the page plans it in the same breath as the books job it follows; a books job is still refused by a running pull, exactly as it was. `enqueue`'s "the same job once" is now keyed on (kind, slugs) so a language job and a top-up over the same slugs cannot shadow each other.
- **The auto run's ledgers merge over DRIVE, whichever way the books came.** The three are two devices' shared record and Drive is the one place both reach; the LAN's merge is `settings.js::runSync`'s own POSTs to Studio and has never been drive.js's. A device that is paired but not signed in therefore merges nothing automatically — named in §7, not hidden.
- **The pull is started BEFORE the ledgers.** The press does the ledgers first, but the press is a person watching a row. `pull.start` is one IPC call that spawns a thread: starting it first means the books are arriving while the page is still on its first Drive round-trip, instead of three round-trips later. The ledgers are not delayed by it — the thread is not this page.
- **`drive.js` names the reader's four keys now**, for the app's ask and nothing else. The file's rule ("this file names no key of the reader's") was already broken once, deliberately and in writing, by `SYNC_PAIR_KEY` on 11 Sep, for exactly this reason: the app's ask lands on whatever page is showing, and the Library and the reader have no Settings to hand a store in. `settings.js` still hands its own in on every press, so the press path is unchanged; the two stores are held equal by USING both over one storage rather than by pinning strings.
- **`jpg` and not `jpeg`.** `parser/cover.py` writes one name, `cover.jpg`. A second extension in the match would be a name nothing in this project produces.
- **The phone shell was imported from a `git archive` of HEAD**, not from TTSTV's working tree, because another lane holds six shell files dirty and `build_shell` copies whole files. G-SYNCBG's workaround, and CLAUDE.md's.

### 4. Boundary check
TTSTV: `library/drive.js`, `library/tests/test_sync_topup.py` (new), `library/STATUS.md`, one line in `PROMPTS/INTENT.md`. `core/` untouched. `clean/` NOT touched (§6). Not touched: `library/import.js`, `library.html`, `library.css`, `surface.js`, `book-nav.js`, `transfer.js`, `settings/settings.js`, `studio/`, `design/`.
Phone repo: `src-tauri/src/lib.rs`, `src-tauri/src/pull.rs`, `tests/test_sync_pull.py`, `shell/` ×8 + `shell.manifest.json` (by `import_shell.py`), `STATUS.md`. Two repos because the prompt names the door and the page; not a move.
**Found dirty and left alone** (another lane's, in TTSTV): `design/reader/{bench-typo.html,book-nav.js,page.css,page.js,test-book-nav.js}`, `reader/{book-nav.js,float.js,floatdoor.js,page.css,page.js}`, staged `reader/reader.html`, `parser/{readers.py,segment.py}`, `studio/tests/test_sync.py`, untracked `parser/paragraphs.py`, `reader/tests/test_float_axis.py`. Phone: `scratch-lookup/*`, `src-tauri/gen/apple/*` ×4, untracked `src-tauri/gen/apple/FrankShare/Info.plist`. TTSTV's HEAD also moved between my first `git status` and my commit (`core/` and `studio/` files that were dirty are now committed by that lane); nothing it touched is tested here.

### 5. Footprint
- **TTSTV `scratch/gtopup/`** (gitignored): `js.tar.gz` 1.3 MB, `js2.tar.gz` 23 MB, `drive.js.orig` 41 KB — the container's test inputs and the RED control. The bridge cannot delete inside the repo; Osca empties it.
- **Phone `_to_delete/gtopup/`**: `phone.tar.gz` 1.0 MB, `phone2.tar.gz` **409 MB**, `phone3.tar.gz` **408 MB** (two attempts at a whole-repo tar that were over the stage cap — the third, `phone4.tar.gz` 191 KB, is the one used), `pull.rs.head`, `lib.rs.head`, `test_sync_pull.py.head`. **Osca: `phone2`/`phone3` are 817 MB and worth deleting first.** Also `_to_delete/shell.1789350498` (the old shell, `import_shell.py`'s rmtree).
- **Git locks the bridge could not unlink, moved** (§8b).
- VM session home (outside both repos, gone with the session): `$HOME/shadow` 57 MB (the HEAD archive + its `clean/` and `out/shell`), `$HOME/g-topup/` (the commit messages and the extractor).
- Container: `/tmp/claude-0/{h,t,b,p}` — the Rust harness and its target, and three checkouts of the staged tars.
- No SSD, no depot write, no env, no model. 0 GPU-minutes.

### 6. Requests to core / other modules
- **`clean/` is OWED, and it is not mine to push.** Another lane holds six shell files dirty (§4), and `export.py --shell` rebuilds `clean/` from the working tree as it stands. Whoever lands those files should run `python3 design/export.py --shell` after committing them: it will carry `library/drive.js` with no clobber, and the shadow build above says the answer is 0 DRIFT.
- **`settings/settings.js` (settings lane)**: `syncPullLine` paints `status.kind` `""` and `"language"`. A top-up run now paints as a books pull ("Pulling 1 of 1 · Hamlet · 1/1"), which is not wrong but is not what it is. A third word for `kind === "topup"` — "Covers…" — is that file's to choose.
- **`library/library.html` (the Library's own words)**: the same status, and the tile the top-up just gave a cover to does not repaint until the page is drawn again. A repaint on `ttstv:sync` ending would close that.
- **`reader/tools/export_bundle.py` (reader)**: still owed from G-COVERS — carry `cover.jpg` into a bundle `.zip`.

### 7. Known gaps
- **`cargo test` on the real crate has still not been run** (G-SYNCBG owed it too), nor an iOS build. The harness proves the logic against the real dependencies; it does not prove the crate compiles for `aarch64-apple-ios`.
- **A device that is paired but NOT signed in to Drive merges no ledgers automatically** — its books come over the LAN and its marks and position wait for a press. The LAN merge is settings.js's and moving it here was not this round's.
- **Nothing was pressed.** No real top-up has been fetched; the proof is a fake network and a real filesystem.
- A top-up of a book whose row this device cannot read (no `.meta.json`, or one that is not JSON) is a sentence and the whole run stops there — the same shape as any other failed book, but a cover is a smaller thing to stop a sync for. Whether it should be skipped instead is Osca's.
- The auto run's ledgers are merged AFTER the pull is started but IN the page: a navigation during those three round-trips drops them, and the next foreground asks again. Same as the plan itself has been since G-SYNCBG.
- `import.js`'s `topUps` and drive.js's copy of it are held equal by a test, not by one implementation. Two files, one rule — the price of the reader having no import.js.

### 8. Next — Osca's presses
1. `cd "IOS TTS TV/src-tauri" && cargo test` — expect the old tests plus `pull::pull_tests` 16 (four new) and `book_tests`'s type test with two more lines.
2. Build to the phone (`tools/phone.sh`; the shell is already imported at v55).
3. **The covers.** Settings ▸ Sync ▸ **Sync**, or just launch the app. The 26 books take their covers on that sync — a cover each, ~40 KB, after the books job — and the shelf shows pictures, not white slabs. Xcode console filter: `frank: topup`.
4. **The position, with nothing pressed.** Set a position on the Mac, press Sync there, then bring Frank to the foreground on the phone without touching anything. Open that book: it is where the Mac left it.
