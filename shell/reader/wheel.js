/* ENGINE · the chapter barrel: which row is live, where each row sits, how it turns
   · bench: bench-wheel.html · mounted by: see MAP.md (generated -- `python3 map.py`) */
...
/* EVERY NUMBER THIS FILE KEEPS, IN ONE PLACE -- and one way in.
   bench-wheel.html is the bench for these the way bench-panes.html is the bench for
   pane.js's, and the rule is the same one PLAN.md already names: a bench that
   holds its OWN copy of the numbers drifts from the app within a day. So the
   bench does not own them; it dials THESE, through configure() below, and the
   app and the bench are the same code with the same values.
   DEFAULTS is the record; CFG is what runs, and it starts as a copy of the
   record -- so a page that never calls configure() behaves exactly as this
   file did before configure() existed. */
const DEFAULTS = {
  /* k stiffness · d damping · c coupling · axis the springs turn about */
  rib: {
  ...
  amps:  [0.018, 0.032, 0.055],   /* light · medium · strong               */
  /* WHERE A WHEEL STARTS -- and there is now only one answer. Osca, 5 Sep:
     "one set of numbers, the bench starts where the app is." shell.html was
     writing --radius:60 over tokens.css's 2.6 and calling turn("none")/amp(0)
     over this file's own paper/medium, so the shipped app had never once shown
     the values either file held. These three ARE the app's, so the bench opens
     on them, the shell needs no override, and there is nothing left to keep in
     step. turn and amp are read at mount (localStorage first, then these), so
     changing them here sets what the NEXT wheel comes up as; a standing one is
     still turned by its own mode()/amp(). */
  radius: 60,      /* R = pitch * this. R/pitch IS the convexity, and 60 is
                      a barrel so large its own sine is straight -- which is
                      the "no convexity in the contents" Osca asked for. Read
                      off --radius when that is set (the tuner and the bench's
                      slider both write it live); this is the value when it is
                      not, and it is the only place the number lives now.   */
  turn:  "none",   /* which ribbon a wheel comes up with: none = no springs
                      per row at all, the block itself is what moves        */
  amp:   0,        /* which of amps[] a wheel comes up on                   */
  grav:  0.20,   /* how hard it centres                                    */
  gravV: 7,      /* the speed at which gravity gives up, px per 1/60s      */
  vmax:  240,    /* the ceiling on coasting speed                          */
  coast: 0.55,   /* how much of that speed the string actually travels     */
  snap:  0.62,   /* a goto -- a click, an arrow key -- closes by this much
                    of what is left, per 1/60s. Lower is harder.           */
};
function copy(d){
  const c = Object.assign({}, d);
  c.rib = {}; for(const k in d.rib) c.rib[k] = d.rib[k] && Object.assign({}, d.rib[k]);
  c.amps = d.amps.slice();
  return c;
}
const CFG  = copy(DEFAULTS);
/* the two the frame loop reads by name every frame -- held, never replaced,
   so configure() can change what is IN them and a mounted wheel sees it */
const RIB  = CFG.rib;
const AMPS = CFG.amps;

/* every mounted wheel's own run(), so a number changed here is on the screen
   this frame rather than the next time somebody touches the string */
const LIVE = new Set();

/* ONE WAY IN. configure() with nothing returns what is running; with a
   partial it merges and wakes every wheel. This is the whole surface
   bench-wheel.html drives, and there is no second copy of a number anywhere. */
function configure(cfg){
  if(cfg){
    if(cfg.rib) for(const k in cfg.rib){
      const v = cfg.rib[k];
      if(v == null){ RIB[k] = null; continue; }
      RIB[k] = Object.assign({axis:"y"}, RIB[k] || {}, v);
    }
    if(cfg.amps){ AMPS.length = 0; cfg.amps.forEach(v => AMPS.push(+v)); }
    ["grav","gravV","vmax","coast","snap","radius"].forEach(k => {
      if(cfg[k] != null) CFG[k] = +cfg[k];
    });
    if(cfg.turn != null && cfg.turn in RIB) CFG.turn = cfg.turn;
    if(cfg.amp  != null) CFG.amp = +cfg.amp;
  }
  LIVE.forEach(wake => { try{ wake(); }catch(_){} });
  return CFG;
}
...
function mount(opts){
  ...
  let TURN=CFG.turn, AMP=CFG.amp;
  try{ TURN=localStorage.getItem("turn")||CFG.turn;
       AMP=+(localStorage.getItem("amp")??CFG.amp); }catch(_){}
  if(!(TURN in RIB)) TURN=CFG.turn;


  const css = n => parseFloat(getComputedStyle(document.documentElement)
                              .getPropertyValue(n));
  const PITCH  = () => css("--pitch")  || 168;
  const RADIUS = () => PITCH() * (css("--radius") || CFG.radius);
  const N      = () => ROWS.length;
  const SPAN   = () => N()*PITCH();
  ...
  function frame(now){
    ...
    if(snapping){                       /* a goto: a click, or an arrow key */
      const d=snapTo-off;
      off += d*(1-Math.pow(CFG.snap,dt)); vel *= Math.pow(0.70,dt);
      ...
      if(!drag && !caught && driver==="wheel"){
        ...
        const s = av>=CFG.gravV ? 0 : 1-av/CFG.gravV;
        if(s>0){ const d=Math.round(off/P)*P - off;
                 off += d*(1-Math.pow(1-CFG.grav*s, dt)); }
      }
    }
  ...
  LIVE.add(run);        // configure() wakes this wheel through it
...
window.Wheel = {mount, configure, DEFAULTS};
})();
