...
function createTrigger({
  ...
  // A NUMBER OR A FUNCTION THAT RETURNS ONE (5 Sep). It is a setting now --
  // Settings > General > Voice > Listening window -- and a setting changed
  // while the reader is open has to reach the next window, not the next page
  // load. A number still works and is still the whole of what tests pass.
  listenWindowMs = 2500,
  onListenStart = () => {},
  onSilence = () => {},
} = {}) {
  ...
  // asked at the moment the window opens, never captured at construction
  function windowMs() {
    const v = typeof listenWindowMs === "function" ? listenWindowMs() : listenWindowMs;
    const n = Number(v);
    return isFinite(n) && n > 0 ? n : 2500;
  }
  ...
  return {
    ...
    // what the next window will be, in ms -- the store's answer, asked now
    windowMs,
