...
function formatAnswer(entry, surfaceWord) {
  ...
  // A SPEAKER'S NAME IS NOT A WORD TO GLOSS (5 Sep). The current export
  // answers cast names out of the book itself -- `source: "book"`, `cue:
  // true`, gloss ["MELIBOEUS — a speaker in this book (7 speeches)."]. Run
  // through the frame below that becomes "MELIBOEUS is 'MELIBOEUS — a speaker
  // in this book (7 speeches)'.", which says the name three times in one
  // breath. The gloss is already a whole sentence about the name, so it is
  // the answer.
  if (entry.cue && entry.source === "book" && sense) {
    return sense + ".";
  }
